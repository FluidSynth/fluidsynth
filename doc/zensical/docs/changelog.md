...
## What's new in 2.6.0?
- A lookahead limiter has been added, see related setting `synth.limiter.*`
- Support for 24bit and 32bit audio has been added, see fluid_synth_write_s24() and fluid_synth_write_s32()

## What's new in 2.5.0?
- [`FLUID_MOD_SIN`](../api/index.md) is now deprecated, use the newly added fluid_mod_set_custom_mapping()
- A new mode for the custom IIR filter has been added: [`FLUID_IIR_BEANLAND`](../api/index.md)
- fluid_mod_get_transform() now receives a `const` argument
- fluid_version_str() now returns a `const` char array
...
## What's new in 2.4.5?
- In order to use the sdl3 audio driver, the downstream application is responsible for calling `SDL_Init()` and `SDL_Quit()`, just like it was practice for the sdl2 audio driver. Fluidsynth may raise a warning if this isn't done, see [Creating the audio driver](usage/audio-driver.md)
...
- #fluid_file_callbacks_t <span style="color:red">now uses `long long` as file-offset type (see #fluid_long_long_t).</span><span style="color:red;font-weight:bold">This is a breaking change</span>, which allows to load SoundFonts bigger than 2GiB on Windows. This change required to bump fluidsynth's SOVERSION.
- <span style="color:red">various fluid_event_*() functions that received a "value" argument of type `short` now receive an `int` argument</span> in preparation for MIDI 2.0 support
...
- [`synth.sample-rate`](../settings/synth.md#settings_synth_sample-rate) is no real-time setting anymore, see note about fluid_synth_set_sample_rate()
- new reverb engine
...
- fluid_synth_sfunload() was not releasing sample buffers of SoundFont3 files if [`synth.dynamic-sample-loading`](../settings/synth.md#settings_synth_dynamic-sample-loading) was set to FALSE
...
**<span style="color:red">Important changes that may not result in a compilation error but may cause your app to misbehave:</span>**

- all public `fluid_settings_*` functions that return an integer which is not meant to be interpreted as bool consistently return either [`FLUID_OK`](../api/index.md) or [`FLUID_FAILED`](../api/index.md)
...
- use unique device names for the [`audio.portaudio_device`](../settings/audio.md#settings_audio_portaudio_device) setting
...
- add [`midi.autoconnect`](../settings/midi.md#settings_midi_autoconnect) a setting for automatically connecting fluidsynth to available MIDI input ports
- add [`synth.overflow_important`](../settings/synth.md#settings_synth_overflow_important) and [`synth.overflow_important-channels`](../settings/synth.md#settings_synth_overflow_important-channels) settings to take midi channels during overflow calculation into account that are considered to be "important"
- add [`synth.dynamic-sample-loading`](../settings/synth.md#settings_synth_dynamic-sample-loading) a setting for enabling on demand sample loading
...
- add realtime settings for [`synth.reverb_damp`](../settings/synth.md#settings_synth_reverb_damp) and [`synth.chorus_depth`](../settings/synth.md#settings_synth_chorus_depth) parameters
...
- complete rewrite of the soundfont loader API, see sfont.h
  ...
  - expose new_fluid_defsfloader() to support loading soundfonts from memory, see fluid_sfloader_set_callbacks() and [fluidsynth_sfload_mem.c](fluidsynth_sfload_mem_8c-example.html)
...
- add a custom sinusoidal modulator mapping function, see [`FLUID_MOD_SIN`](../api/index.md)
