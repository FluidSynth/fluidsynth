...
/*      printf("track=%d\tticks=%u\ttrack=%u\tdtime=%u\tnext=%u\n",  */
/*  	   track->num,  */
/*  	   ticks,  */
/*  	   track->ticks,  */
/*  	   event->dtime,  */
...
int fluid_player_callback(void* data, unsigned int msec)
{
  ...
  delta_ticks = (unsigned int) (((double) msec - (double) player->msec_passed) 
				/ player->deltatime);
...
int fluid_player_set_midi_tempo(fluid_player_t* player, int tempo)
{
  ...
  FLUID_LOG(FLUID_DBG,"tempo=%d\n", tempo);
  FLUID_LOG(FLUID_DBG,"tick time=%f msec\n", player->deltatime);
